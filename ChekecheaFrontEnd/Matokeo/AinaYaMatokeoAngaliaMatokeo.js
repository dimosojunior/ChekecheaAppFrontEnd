...
const AinaYaMatokeoAngaliaMatokeo = ({ navigation, route }) => {
...
const getItems = (token) => {
  if (endReached) {
   ...
   const url = EndPoint + `/GetMakundiMatokeoYaSemisterAngaliaMatokeoView/?Darasa=${Darasa}&page=${current_page}&page_size=500`
    // console.log(url);
    fetch(url, {
      method: 'GET',
      headers: {
...
const handleRefresh = async () => {
  ...
  try {
    ...
    if (token) {
      ...
      const url = EndPoint + `/GetMakundiMatokeoYaSemisterAngaliaMatokeoView/?Darasa=${Darasa}&page=1&page_size=500`;
...
const handlePressDetailsPage = (item) =>
    navigation.navigate('Angalia Matokeo Ya Mwanafunzi Mmoja',
     { ...item,Mkondo,
      Darasa,
     KiasiChaAda,
...
const TableRowComponent = ({ item}) => {
  ...
  return (
    <View key={item.id} style={globalStyles.row2}>
      ...
      <Text style={[globalStyles.cell, globalStyles.firstNameColumn]}>{item.Matokeo}</Text>
     
  
    
<Text style={[globalStyles.cell, globalStyles.firstNameColumn]}>{item?.Tarehe}-{item?.Mwezi}-{item?.Year}</Text>
 ...
 if (item.Matokeo.toLowerCase().includes(input.toLowerCase())) {
...
<Text style={[globalStyles.cell, globalStyles.otherColumns]}>{item?.Tarehe}-{item?.Mwezi}-{item?.Year}</Text>
      ...
      <>
              ...
              <View style={globalStyles.table}>
                <View style={[globalStyles.row, globalStyles.header]}>
                 ...
                 <Text style={[globalStyles.cell2, globalStyles.buttoncolumn]}>Tazama</Text>
                 
                  <Text style={[globalStyles.cell2, globalStyles.firstNameColumn]}>Matokeo</Text>
                 
                 <Text style={[globalStyles.cell2, globalStyles.firstNameColumn]}>Muda</Text>
...
export default AinaYaMatokeoAngaliaMatokeo;
