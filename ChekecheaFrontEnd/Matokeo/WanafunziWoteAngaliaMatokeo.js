...
const WanafunziWoteAngaliaMatokeo = ({navigation, route}) => {
...
const getItems = async (token) => {
  if (endReached) {
   ...
   const url = EndPoint + `/GetWanafunziWoteHaiAngaliaMatokeo/?Darasa=${Darasa}&Mkondo=${Mkondo}&page=${current_page}&page_size=500`
    // console.log(url);
    fetch(url, {
      method: 'GET',
      headers: {
...
const handleRefresh = async () => {
  ...
  try {
    ...
    if (token) {
      ...
      const url = EndPoint + `/GetWanafunziWoteHaiKwaMatokeo/?Mwezi=${Mwezi}&Tarehe=${Tarehe}&Year=${Year}&Matokeo=${Matokeo}&Darasa=${Darasa}&Mkondo=${Mkondo}&page=1&page_size=500`;
      const response = await fetch(url, {
        method: 'GET',
        headers: {
          Authorization: `Token ${token}`,
...
const handlePressPokeaMarejesho = (item) =>
    navigation.navigate('Aina Ya Matokeo Angalia Matokeo',
     { ...item,
      Darasa,Mkondo
       });
 ...
 {!isPending ? (
      ...
      <>
              ...
              <View style={globalStyles.table}>
                ...
                <Text style={[globalStyles.cell2, globalStyles.buttoncolumn]}>Endelea</Text>
...
export default WanafunziWoteAngaliaMatokeo;
