...
const AinaYaMatokeo = ({ navigation, route }) => {
...
const getItems = (token) => {
  if (endReached) {
   ...
   const url = EndPoint + `/GetMakundiMatokeoYaSemisterView/?Darasa=${Darasa}&page=${current_page}&page_size=500`
    // console.log(url);
    fetch(url, {
      method: 'GET',
      headers: {
...
const handleRefresh = async () => {
  ...
  try {
    ...
    if (token) {
      ...
      const url = EndPoint + `/GetMakundiMatokeoYaSemisterView/?Darasa=${Darasa}&page=1&page_size=500`;
...
const handlePressDetailsPage = (item) =>
    navigation.navigate('Wanafunzi Wote Matokeo', { ...item,Mkondo, Darasa, KiasiChaAda });
