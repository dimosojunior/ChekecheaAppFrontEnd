...
const WanafunziWoteMatokeo = ({navigation, route}) => {
...
const getItems = async (token) => {
  if (endReached) {
   ...
   const url = EndPoint + `/GetWanafunziWoteHaiKwaMatokeo/?Mwezi=${Mwezi}&Tarehe=${Tarehe}&Year=${Year}&Matokeo=${Matokeo}&Darasa=${Darasa}&Mkondo=${Mkondo}&page=${current_page}&page_size=500`
    // console.log(url);
    fetch(url, {
      method: 'GET',
      headers: {
...
const handlePressPokeaMarejesho = (item) =>
    navigation.navigate('Ingiza Matokeo',
     { ...item,
      Darasa,Mkondo,
      Matokeo,Mwezi,Tarehe,Year
       });
...
export default WanafunziWoteMatokeo;
