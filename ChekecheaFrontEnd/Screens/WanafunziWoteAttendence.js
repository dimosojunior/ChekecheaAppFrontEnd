...
const WanafunziWoteAttendence = ({navigation, route}) => {
...
const getItems = async (token) => {
  if (endReached) {
   ...
   const url = EndPoint + `/GetWanafunziWoteHai/?Darasa=${Darasa}&Mkondo=${Mkondo}&page=${current_page}&page_size=500`
    // console.log(url);
    fetch(url, {
      method: 'GET',
      headers: {
...
const handleRefresh = async () => {
  ...
  try {
    ...
    if (token) {
      ...
      const url = EndPoint + `/GetWanafunziWoteHai/?Darasa=${Darasa}&Mkondo=${Mkondo}&page=1&page_size=500`;
      const response = await fetch(url, {
        method: 'GET',
        headers: {
          Authorization: `Token ${token}`,
...
const handlePressPokeaMarejesho = (item) =>
    navigation.navigate('Ita Attendence', { ...item,Darasa,Mkondo });
 ...
 {!isPending ? (
      ...
      <>
              ...
              <View style={globalStyles.table}>
                ...
                <Text style={[globalStyles.cell2, globalStyles.buttoncolumn]}>Ita</Text>
  ...
  {/*<Text style={{
    color:'white',
        ...
        {/*  <View style={{
         ...
         //onPress={() => navigation.navigate("Home Stack")}
          onPress={(
            ) => navigation.navigate('Funga Attendence', { Darasa: Darasa, Mkondo: Mkondo })}
...
export default WanafunziWoteAttendence;
